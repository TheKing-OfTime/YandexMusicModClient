...
В ванильном клиенте есть набор багов которые довольно сильно ухудщают опыт использования. Некоторые из них исправлены в моде. Например:

- Клиент больше не теряет чёткость если занимает не чётное количество пикселей по высоте или ширине.
- Клиент больше не теряет медиаконтекст (ака медиаклавиши) когда включён кроссфейд.
- Воспроизведение AAC треков не застревает если перемотать на 1:00 ровно. 
...
- В рамках улучшения анимации Волны обновлены неверные параметры анализатора (если точнее правильно настроены fftSize и smoothingTimeConstant)
