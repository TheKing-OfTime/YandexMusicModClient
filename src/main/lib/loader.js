  ...
  try {
    ...
    if (result.isDirectory()) {
      return await resolvePath(
        node_path_1.default.join(normalizedFilePath, "index.html"),
      );
    }
  } catch (error) {}
  return null;
...
const loader = (options) => {
  ...
  serveOptions.buildPath = node_path_1.default.resolve(
  ...
  const fileProtocolHandler = async (request, callback) => {
    ...
    if (
      ...
      const fallbackIndexPath = node_path_1.default.join(
        serveOptions.buildPath,
  ...
  electron_1.app.on("ready", () => {
    electron_1.session.defaultSession.protocol.registerFileProtocol(
