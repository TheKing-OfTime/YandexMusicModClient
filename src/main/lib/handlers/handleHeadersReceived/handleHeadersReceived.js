...
const handleHeadersReceived = (window) => {
  ...
  if (
    ![platform_js_1.Platform.MACOS, platform_js_1.Platform.WINDOWS].includes(
      ...
      (details, callback) => {
        ...
        if (headers) {
          if (headers["x-yandex-music-client"]) {
            newHeaders["x-yandex-music-client"] =
              "YandexMusicDesktopAppWindows/" +
              headers["x-yandex-music-client"].split("/")[1];
          }
        }
